...
from HPA_CC.models.DINO4Cells.archs import vision_transformer as vits
from HPA_CC.models.DINO4Cells.archs.vision_transformer import DINOHead
from HPA_CC.models.DINO4Cells.archs import xresnet as cell_models  # (!)
from HPA_CC.models.DINO4Cells.utils import utils
...
class DINO(nn.Module):
    ...
    def __init__(self,
        ...
        super().__init__()
        self.dinov2 = torch.hub.load('facebookresearch/dinov2', 'dinov2_vitl14', verbose=silent)
    ...
    def __get_crop_slices(imsize, margin_tolerance, dino_patch_size):
        ...
        for image_size in imsize:
            ...
            if not silent: print(f"Margin cropped out to fit DINO patches: {margin_size}")
...
class DINO_HPA:
    ...
    def predict_cls_ref(self, sample):
        ...
        assert len(sample.shape) == 4, f"sample must have B x C x H x W shape, got {sample.shape} instead"
        assert sample.shape[1] == 2, f"sample must have 2 channels only (dapi+microtubule), got {sample.shape[1]} instead"
